# Azure Solutions Architect Expert (AZ-305) Study Notes

A comprehensive collection of study notes and documentation for the Microsoft Azure Solutions Architect Expert (AZ-305) certification exam.

## Overview

This repository contains structured notes covering Azure services, architectural concepts, and best practices required for the AZ-305 certification. The content is organized by topic areas with a focus on practical understanding and exam preparation.

## Table of Contents

### 00. Basic Concepts
- [Regions](./00-basic-concepts/regions.md) - Region selection, Availability Zones, geographical considerations
- [Resource Groups](./00-basic-concepts/resource-group.md) - Logical containers, naming conventions, subscriptions vs management groups
- [Cost Management](./00-basic-concepts/cost.md) - Pricing models, budgets, Azure Calculator
- [SLA (Service Level Agreement)](./00-basic-concepts/sla.md) - Uptime calculations, SLA examples
- [Storage Accounts](./00-basic-concepts/storage-account.md) - Azure storage fundamentals

### 01. Azure Compute
- [Compute Overview](./01-azure-compute/compute.md) - Types of compute services in Azure
- [Virtual Machines](./01-azure-compute/vm.md) - VMs, cost optimization, availability, VMSS, ARM templates
- [App Service](./01-azure-compute/app-service.md) - Fully managed web hosting, deployment slots, tiers, auto-scaling
- [AKS (Azure Kubernetes Service)](./01-azure-compute/aks.md) - Managed Kubernetes service, container orchestration
- [Azure Functions](./01-azure-compute/azure-functions.md) - Serverless compute, event-driven execution, consumption plans

### 02. Azure Networking
- [Virtual Networks (VNets)](./02-azure-networking/virtual-networks.md) - VNet fundamentals, subnets, address spaces, peering